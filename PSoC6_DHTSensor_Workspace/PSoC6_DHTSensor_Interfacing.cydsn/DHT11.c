...
uint32_t expectPulse(uint32_t pinState){
    ...
    if(pinState == 0){
        while(Cy_GPIO_Read(DHT_DATA_PORT, DHT_DATA_NUM) == 0UL){
            count++;
        ...
        while(Cy_GPIO_Read(DHT_DATA_PORT, DHT_DATA_NUM) == 1UL){
            count++;
            if(count > MIN_INTERVAL){
                count = TIMEOUT;
                break;
...
uint8* DHT_Read(){
    ...
    Cy_GPIO_SetDrivemode(DHT_DATA_PORT, DHT_DATA_NUM, CY_GPIO_DM_STRONG);
    
    Cy_GPIO_Write(DHT_DATA_PORT, DHT_DATA_NUM, 0);
    Cy_SysLib_Delay(RETENTION_TIME);
    Cy_GPIO_Write(DHT_DATA_PORT, DHT_DATA_NUM, 1);
    
    Cy_GPIO_SetDrivemode(DHT_DATA_PORT, DHT_DATA_NUM, CY_GPIO_DM_PULLUP);
